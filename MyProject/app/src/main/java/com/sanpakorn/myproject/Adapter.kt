...
class Adapter (act : FragmentActivity, val dataSource: JSONArray) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (convertView == null) {
            ...
            // 3
            holder = ViewHolder()
            holder.userTextView = view.findViewById(R.id.name) as TextView
            holder.detailTextView = view.findViewById(R.id.des) as TextView
        ...
        userTextView.setText( dataSource.getJSONObject(position).getString("name").toString() )
        detailTextView.setText( dataSource.getJSONObject(position).getString("des").toString() )
